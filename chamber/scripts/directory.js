...
const LEVEL_CLASSES = { 1: 'level-member', 2: 'level-silver', 3: 'level-gold' };
...
async function loadSpotlights() {
  const display = document.querySelector('#spotlights-display');
  if (!display) return;
  ...
  try {
    ...
    display.innerHTML = picks.map(buildSpotlightCard).join('');

  } catch (err) {
    display.innerHTML = `<p class="error-msg">Spotlight data unavailable. Please try again later.</p>`;
  ...
  return `
    <div class="member-card">
        ...
        <div class="member-card-body">
            ...
            <span class="membership-badge ${levelClass}">
                ${levelLabel}
            </span>
